    ...
    <div class="flex">
        <div class="mr-4">
            <x-jet-secondary-button disabled x-bind:disabled="$wire.qty <= 1" {{-- Condicion para deshabilitar disabled  --}}
                wire:loading.attr="disabled" {{-- Condicion evitar que baje a negeativos al spamear al - --}} wire:target="decrement"
            ...
            <x-jet-secondary-button disabled x-bind:disabled="$wire.qty >= $wire.quantity" {{-- Condicion para deshabilitar disabled  --}}
                wire:loading.attr="disabled" {{-- Condicion evitar que baje a negeativos al spamear al - --}} wire:target="increment"
                wire:click="increment">+</x-jet-secondary-button>
        </div>

        <div class="flex-1"> {{-- Todo el ancho posible con flex-1   --}}
            <x-jet-button x-bind:disabled="$wire.qty > $wire.quantity" {{-- Condicion para deshabilitar disabled  --}}
                class="w-full justify-center" wire:click="addItem" {{-- Al dar clik, llama al método --}} wire:loading.attr="disabled"
                wire:target="addItem" style="background-color: #08769a; color: white;">
